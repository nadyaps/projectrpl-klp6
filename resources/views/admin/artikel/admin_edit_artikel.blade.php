  ...
  <div class="col-md-12 col-xl-12 center-wrapper">
    <div class="card rounded">
      <div class="card-body">
        ...
        <form method="POST" action="{{route('store.artikel', $table->id)}}" class="forms-sample" enctype="multipart/form-data" class="forms-sample">
        @csrf
          <div class="row mb-3">
            <label for="exampleInputUsername2" class="col-sm-3 col-form-label">Judul</label>
            <div class="col-sm-9">
              <input type="text" name="judul" class="form-control @error('judul') is-invalid @enderror" value ="{{$table->judul}}">
              @error('judul')
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-9">
              <input name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" value ="{{$table->deskripsi}}" >
              @error('deskripsi')
                <span class="text-danger">{{ $message }}</span>
              @enderror
            </div>
          ...
          <div class="row mb-3">
            ...
            <img  id="showImage" class="wd-150" src="{{ (!empty ($table->photo)) ? url('admin_image_artikel/' .$table->photo) :url('no_image.jpg')}}" alt="photo">
          </div>
